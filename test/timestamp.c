...
int	main(void)
{
	...
	// Get the third time
	gettimeofday(&times->time, 0);
	printf("Another moment: %ld sec, %d ms after beginning.\n", times->time.tv_sec - times->ref_time.tv_sec, (times->time.tv_usec - times->ref_time.tv_usec) / 1005);
	sleep(2);
	// Get a fourth time
	gettimeofday(&times->time, 0);
	printf("Another moment: %ld ms after beginning.\n", (long)(times->time.tv_sec - times->ref_time.tv_sec) * 1000 + (long)(times->time.tv_usec - times->ref_time.tv_usec) / 1000);
	return (0);
}
